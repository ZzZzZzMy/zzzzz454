...
//mock数据操作，需要引入url记忆fs，但是url/fs是内置的，所以不需要安装
var url = require('url');
...
/*
1.创建src目录(src是开发目录，所有操作都在src中进行)
2.在src新建index.html(因为我们现在做的是spa项目，所有，通常我们只有一个入口主文件)
3.实现index.html的文件复制操作，复制的目标是www
4.webserver的本地服务器配置(不是gulp-connect)
5.实现mock数据操作，先在根目录下创建mock目录
6.实现sass转换
7.实现js的模块化操作
8.版本管理
...
gulp.task('webserver',function(){
	...
	.pipe(webserver({
		...
		middleware:function(req,res,next){
			...
			//如果url里面输出了/skill.php,/project.php或者是/work，那么我们就可以查找到urlObj.pathname为skill.php,/project.php,/word
			...
			switch(urlObj.pathname){
				...
				//需要通过fileSystem文件操作函数，去读取指定目录下的json文件，并将读取到大内容返回到浏览器端
...
gulp.task('html',function(){
	//src第一个参数是我们的映射资源文件，第二个参数使我们需要替换的html文件
